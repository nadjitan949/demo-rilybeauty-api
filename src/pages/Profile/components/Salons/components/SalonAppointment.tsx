...
function SalonAppointment() {
    ...
    const confirmCashPayment = async (id: number) => {
        try {
            ...
            setConfirmedPayment(res.data.confirmePayment)
    ...
    return (
        <>
            ...
            {showReceiptModal && confirmedPayment && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <section className="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative overflow-hidden">

                        {/* Header du reçu */}
                        <div className="p-6 border-b border-slate-100 bg-linear-to-r from-green-50 to-emerald-50 text-center">
                            <div className="w-16 h-16 rounded-full bg-green-100 flex items-center justify-center mx-auto mb-3">
                                <span className="text-3xl">✅</span>
                            </div>
                            <h2 className="text-xl font-bold text-slate-800">Paiement confirmé !</h2>
                            <p className="text-sm text-slate-500 mt-1">Voici votre reçu de transaction</p>
                        </div>

                        {/* Contenu du reçu (style papier) */}
                        <div className="p-6 space-y-4 bg-white">

                            {/* En-tête du salon */}
                            <div className="text-center pb-4 border-b border-dashed border-slate-200">
                                <p className="font-bold text-slate-800 text-lg">{user?.salon?.name}</p>
                                <p className="text-xs text-slate-500">{user?.salon?.address}, {user?.salon?.city}</p>
                                <p className="text-xs text-slate-500">{user?.salon?.country}</p>
                            </div>

                            {/* Détails du rendez-vous */}
                            <div className="space-y-3 text-sm">
                                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                                    <span className="text-slate-500">Référence RDV</span>
                                    <span className="font-mono font-bold text-slate-800">#{confirmedPayment?.appointment?.reference}</span>
                                </div>
                                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                                    <span className="text-slate-500">Date & Heure</span>
                                    <span className="font-medium text-slate-800">
                                        {confirmedPayment?.appointment?.date ? new Date(confirmedPayment.appointment.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : '-'}
                                    </span>
                                </div>
                                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                                    <span className="text-slate-500">Service</span>
                                    <span className="font-medium text-slate-800">{confirmedPayment?.appointment?.service?.name}</span>
                                </div>
                                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                                    <span className="text-slate-500">Employé</span>
                                    <span className="font-medium text-slate-800">{confirmedPayment?.appointment?.employee?.firstname} {confirmedPayment?.appointment?.employee?.lastname}</span>
                                </div>
                                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                                    <span className="text-slate-500">Client</span>
                                    <span className="font-medium text-slate-800">{confirmedPayment?.appointment?.user?.firstname} {confirmedPayment?.appointment?.user?.lastname}</span>
                                </div>
                            </div>

                            {/* Détails du paiement */}
                            <div className="p-4 bg-green-50 rounded-xl border border-green-100 mt-4">
                                <p className="text-xs font-semibold text-green-700 uppercase tracking-wide mb-3 text-center">Détails du paiement</p>
                                <div className="space-y-2 text-sm">
                                    <div className="flex justify-between">
                                        <span className="text-slate-500">Montant</span>
                                        <span className="font-bold text-slate-800">{confirmedPayment?.amount} {confirmedPayment?.currency}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-slate-500">Méthode</span>
                                        <span className="font-medium text-slate-800">{confirmedPayment?.provider}</span>
                                    </div>
                                    <div className="flex justify-between">
                                    ...
                                    {confirmedPayment?.id && (
                                        <div className="flex justify-between pt-2 border-t border-green-100 mt-2">
                                            <span className="text-slate-500 text-xs">ID Transaction</span>
                                            <span className="font-mono text-xs text-slate-600">#{confirmedPayment.id}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="text-center pt-4 border-t border-dashed border-slate-200">
                                ...
                                <p className="text-xs text-slate-400 mt-1">Reçu généré le {new Date().toLocaleDateString('fr-FR')}</p>
                            </div>
                        </div>
                        ...
                        <div className="p-5 border-t border-slate-100 bg-slate-50 flex gap-3">
                            <button
                                ...
                                className="flex-1 py-2.5 rounded-xl border border-slate-300 text-slate-700 hover:bg-slate-100 font-medium transition-colors flex items-center justify-center gap-2"
                            >
                                🖨️ Imprimer
                            </button>
                            <button
                                type="button"
                                onClick={() => { setShowReceiptModal(false); setConfirmedPayment(null); }}
                                className="flex-1 py-2.5 rounded-xl bg-violet-600 hover:bg-violet-700 text-white font-medium shadow-sm hover:shadow transition-all"
                            >
                                Fermer
                            </button>
                        </div>
